...
import static com.blockadm.common.comstomview.stateswitch.StateLayout.State.ERROR;
import static com.blockadm.common.comstomview.stateswitch.StateLayout.State.ING;
...
public class StateLayout extends FrameLayout {
    ...
    private static IngCreator sIngCreator = new IngCreator() {
        @NonNull
        @Override
        public StateIngInterface createStateIng(@NonNull Context context,
												@NonNull StateLayout layout) {
    ...
    private static EmptyCreator sEmptyCreator = new EmptyCreator() {
        @NonNull
        @Override
        public StateEmptyInterface createStateEmpty(@NonNull Context context,
													@NonNull StateLayout layout) {
    ...
    private static ErrorCreator sErrorCreator = new ErrorCreator() {
        @NonNull
        @Override
        public StateErrorInterface createStateError(@NonNull Context context,
													@NonNull StateLayout layout) {
    ...
    private void initAttribute(AttributeSet attrs, int defStyleAttr) {
        TypedArray typeArray = getContext().obtainStyledAttributes(attrs, R.styleable.StateLayout
				, defStyleAttr, 0);
        int contentLayoutId =
				typeArray.getResourceId(R.styleable.StateLayout_state_content_layout, -1);
        typeArray.recycle();
        if (contentLayoutId == -1) {
            throw new NullPointerException("StateSwitchLayout use should set content layout " +
					"id!!!!!!");
        }
    ...
    public void setEmptyStateView(@NonNull StateEmptyInterface stateWrap,
								  StateEmptyInterface.OnRetryListener retryListener) {
