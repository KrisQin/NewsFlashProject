...
import tv.danmaku.ijk.media.player.utils.MediaUtils;
...
public class PlayerView {
    ...
    public  void sendMediaPlayOrPause(Context context, int keyCode) {
...
//                showMenu();
    ...
    public PlayerView onDestroy() {
        ...
        videoView.stopPlayback();
        mAutoPlayRunnable.stop();//停止controller的轮询
        return this;
    }
    ...
    /**
     * 在onOptionsItemSelected方法中调用
     * @param menuItem
     * */
    public void onOptionsItemSelected(MenuItem menuItem){
        if(menuItem.getItemId() == android.R.id.home){//返回键的操作
            if (!isOnlyFullScreen && !isPortrait) {
                mActivity.setRequestedOrientation(ActivityInfo.SCREEN_ORIENTATION_PORTRAIT);
            } else {
                if (mPlayerBack != null) {
    ...
    public PlayerView setPlaySource(String url) {
        setPlaySource(mContext.getString(R.string.standard_definition), url);
        return this;
    }
...
//                    query.id(R.id.app_video_loading).visible();
    ...
    public boolean isLive() {
        if (currentUrl != null
                && (currentUrl.startsWith("rtmp://") || (currentUrl.startsWith("http://") && currentUrl.endsWith(".m3u8")) || (currentUrl.startsWith("http://") && currentUrl.endsWith(".flv")))) {
            isLive = true;
        } else {
            isLive = false;
        }
        ...
        if (newStatus == PlayStateParams.STATE_COMPLETED) {
            ...
            if (!(isGNetWork &&
                ...
                if (isCharge && maxPlaytime < getCurrentPosition()) {
                    ...
                    hideAll();
                    if (isLive) {
                        showStatus(mActivity.getResources().getString(R.string.play_null_error));
                    } else {
    ...
    private void updatePausePlay() {
        if (videoView.isPlaying()) {
            if (isLive) {
                ...
                iv_bar_player.setBackgroundResource(R.drawable.select_simple_player_start_btn);
                iv_player.setBackgroundResource(R.drawable.select_simple_player_center_start_btn);
            }
        } else {
            iv_bar_player.setBackgroundResource(R.drawable.select_simple_player_stop_btn);
            iv_player.setBackgroundResource(R.drawable.select_simple_player_center_stop_btn);
        }
    }
    ...
    private void updateFullScreenButton() {
        if (getScreenOrientation() == ActivityInfo.SCREEN_ORIENTATION_LANDSCAPE) {
            iv_fullscreen.setBackgroundResource(R.drawable.select_simple_player_exit_fullscreen_bar);
        } else {
            iv_fullscreen.setBackgroundResource(R.drawable.select_simple_player_fullscreen_bar);
        }
    }
    ...
    private class AutoPlayRunnable implements Runnable {
        ...
        public void run() {
            if (mShouldAutoPlay) {
                Log.i("PlayerView", "removeCallbacks ----->" + isForbidTouch + "....." + isShowControlPanl);
//                mHandler.removeCallbacks(this);
                // && !isShowControlPanl
                if (!isForbidTouch ) {
                    Log.i("PlayerView", "hideControl ----->");
                    operatorPanl();
                }
            }
        }
